...
class CompatibilityOracle {
  ...
  constructor(data: Compatibility[]) {
    this.data = new Set(data.map((compatibility) => JSON.stringify(compatibility)));
  }
...
class Wavefunction {
  ...
  constructor(size: [number, number], weights: Weights) {
    this.coefficient_matrix = this.init_coefficient_matrix(size, Object.keys(weights))
    this.weights = weights
  }

  private init_coefficient_matrix(size: [number, number], tiles: Tile[]): CoefficientMatrix {
  ...
  collapse(co_ords: Coordinates) {
    ...
    const filtered_tiles_with_weights = Object.entries(this.weights).filter(([title]) => opts.has(title));
    const total_weights = filtered_tiles_with_weights.reduce((acc, [_, weight]) => acc + weight, 0);
...
function valid_dirs(cur_co_ords: Coordinates, matrix_size: [number, number]): Direction[] {
...
class Model {
  ...
  constructor(output_size: [number, number], weights: Weights, compatibility_oracle: CompatibilityOracle) {
